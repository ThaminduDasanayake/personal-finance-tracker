...
export const FinancialRecordEdit = () => {
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      category: "" as
        ...
        | "Entertainment"
        | "Other",
      paymentMethod: "" as "Credit Card" | "Cash" | "Bank Transfer",
    },
  });
  ...
  const { record, updateRecord } = useFinancialRecordsHook(userId, id!);

  useEffect(() => {
    if (record && Object.keys(record).length > 0) {
      const parsedRecord = formSchema.safeParse(record);
      if (parsedRecord.success) {
        const { description, amount, type, date, category, paymentMethod } =
          parsedRecord.data;
        ...
        console.error("Invalid record data", parsedRecord.error);
      }
    }
  }, [record, form]);
      ...
      <Form {...form}>
        ...
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <Button type="button">Update Record</Button>
          </AlertDialogTrigger>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Are you absolutely sure?</AlertDialogTitle>
              <AlertDialogDescription>
                This action cannot be undone. This will update your financial
                record.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
